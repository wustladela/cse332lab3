...
int parse(vector<Card> &vector, string filename)
{
	...
	if (ifs.good())
	{
		...
		while (getline(ifs, line))
		{
			...
			{
				cout << "Warning: Improper number of cards in a hand" << endl;
			}
			temp.clear();
			count = 0;
		}
...
void getAllRanks(const vector<Card> &vector)
{
	std::vector<Card> temp;
	...
	for (int i = 0; i < vector.size() - end_of_hand_range; i = i + hand_range)
	{
		for (int j = i; j < i + hand_range; ++j)
		{
...
int getRank(vector<Card> &vector)
{
